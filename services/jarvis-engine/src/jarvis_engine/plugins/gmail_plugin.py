...
def get_unread_emails(max_results: int = 5) -> list[dict]:
    ...
    resp = httpx.get(
        f"{GMAIL_API_BASE}/messages", headers=headers, params=params, timeout=30.0
    )
    ...
    for msg in messages:
        ...
        detail_resp = httpx.get(
            f"{GMAIL_API_BASE}/messages/{msg_id}?format=metadata",
            headers=headers,
            timeout=30.0,
        )
...
def send_email(to: str, subject: str, body: str) -> bool:
    ...
    resp = httpx.post(
        f"{GMAIL_API_BASE}/messages/send", headers=headers, json=payload, timeout=30.0
    )
    resp.raise_for_status()
