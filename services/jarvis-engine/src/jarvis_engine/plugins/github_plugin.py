...
def list_issues(repo: str, state: str = "open") -> list[dict]:
    ...
    resp = httpx.get(
        f"{GITHUB_API_BASE}/repos/{safe_repo}/issues?state={safe_state}&per_page=100",
        headers=headers,
        timeout=30.0,
    )
    data = _handle_response(resp)
...
def search_issues(query: str) -> list[dict]:
    ...
    resp = httpx.get(
        f"{GITHUB_API_BASE}/search/issues?q={safe_q}", headers=headers, timeout=30.0
    )
    data = _handle_response(resp)
...
def list_pull_requests(repo: str, state: str = "open") -> list[dict]:
    ...
    safe_state = quote(state, safe="")
    resp = httpx.get(
        f"{GITHUB_API_BASE}/repos/{safe_repo}/pulls?state={safe_state}&per_page=100",
        headers=headers,
        timeout=30.0,
    )
    data = _handle_response(resp)
...
def get_pr_status(repo: str, pr_number: int) -> dict:
    ...
    pr_resp = httpx.get(
        f"{GITHUB_API_BASE}/repos/{safe_repo}/pulls/{safe_pr}",
        headers=headers,
        timeout=30.0,
    )
    ...
    status_resp = httpx.get(
        f"{GITHUB_API_BASE}/repos/{safe_repo}/commits/{head_sha}/status",
        headers=headers,
        timeout=30.0,
    )
...
def search_code(query: str, repo: str | None = None) -> list[dict]:
    ...
    safe_q = quote(q, safe="")
    resp = httpx.get(
        f"{GITHUB_API_BASE}/search/code?q={safe_q}", headers=headers, timeout=30.0
    )
    data = _handle_response(resp)
