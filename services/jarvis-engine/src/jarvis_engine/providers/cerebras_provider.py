...
class CerebrasProvider(BaseProvider):
  ...
  @property
  def model(self) -> str:
    return self._selected_model if self._selected_model else self.PREFERRED_MODELS[0]
            ...
            async with httpx.AsyncClient(timeout=10.0) as c:
                ...
                if r.status_code == 200:
                    ...
                    if not self._selected_model:
                        return False
                    print(f"[CEREBRAS] Auto-selected model: {self._selected_model} from available models")
                    return True
                return False
