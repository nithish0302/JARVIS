...
from ..plugins.credential_store import store_credential
from ..providers import fallback as fallback_module
from ..providers.fallback import (
    build_ask_message,
    build_fallback_note,
    build_override_unavailable_message,
    build_unconfigured_message,
    run_cascade,
)
from ..providers.manager import provider_manager
from ..tools.search_detector import extract_search_query, needs_web_search
...
class _PinRateLimiter:
    """In-memory PIN failure tracker, shared process-wide (single instance below)."""

    def __init__(self):
        self.fail_count = 0
        self.fail_window_start = 0.0
    ...
    def record_failure(self):
        now = time.monotonic()
        if now - self.fail_window_start > _PIN_WINDOW_SECONDS:
            self.fail_count = 0
            self.fail_window_start = now
        self.fail_count += 1
        if self.fail_count >= _PIN_MAX_ATTEMPTS:
            self.lockout_until = now + _PIN_LOCKOUT_SECONDS
            self.fail_count = 0
...
_pin_rate_limiter = _PinRateLimiter()
...
async def search_endpoint(request: dict):
    ...
    return {"query": query, "results": results, "formatted": formatted}
...
async def voice_status():
    return {
        ...
        "status": "active" if voice_manager.wake_word_detector else "inactive",
    }
...
async def get_settings_endpoint():
    ...
    address_preference = await get_setting(
        "address_preference", settings.ADDRESS_PREFERENCE
    )
    daily_briefing_enabled = (
        await get_setting("daily_briefing_enabled", settings.DAILY_BRIEFING_ENABLED)
        == "true"
    )
    ...
    gemini_configured = bool(
        _get_cred("provider_config", "GEMINI_API_KEY")
        or await get_setting("GEMINI_API_KEY")
    )
    groq_configured = bool(
        _get_cred("provider_config", "GROQ_API_KEY")
        or await get_setting("GROQ_API_KEY")
    )
    openrouter_configured = bool(
        _get_cred("provider_config", "OPENROUTER_API_KEY")
        or await get_setting("OPENROUTER_API_KEY")
    )
    ollama_configured = bool(
        _get_cred("provider_config", "OLLAMA_HOST") or await get_setting("OLLAMA_HOST")
    )
...
async def verify_delete_pin_endpoint(request: dict):
    ...
    allowed, err = _pin_rate_limiter.check()
    if not allowed:
        raise HTTPException(status_code=429, detail=err)
    stored_hash = await get_setting(
        "conversation_delete_pin", settings.CONVERSATION_DELETE_PIN
    )
    if _verify_pin(pin, stored_hash):
        _pin_rate_limiter.record_success()
        return {"valid": True}
    _pin_rate_limiter.record_failure()
    return {"valid": False}
...
async def update_voice_status(request: dict):
    ...
    await broadcast_voice_event({"type": "voice_status", "status": status})
...
async def voice_input_endpoint(request: dict):
    ...
    await broadcast_voice_event({"type": "voice_status", "status": "processing"})
    ...
    if direct_response:
        ...
        await save_message(conversation_id=conversation_id, role="user", content=text)
        await save_message(
        ...
        await broadcast_voice_event(
            {"type": "voice_input", "text": text, "conversation_id": conversation_id}
        )
        await broadcast_voice_event(
            {
                "type": "voice_response",
    ...
    if is_file_cmd:
        ...
        for result in automation_results:
            ...
            if action_type == "OPEN_APP":
                automation_context += (
                    f"Open {command} → [UI_ACTION:open_app:{command}]\n"
                )
                ...
                if command == "lock_screen":
                    automation_context += "Lock screen → [UI_ACTION:lock_screen]\n"
                elif command.startswith("close_app:"):
                    app = command.replace("close_app:", "").strip()
                    automation_context += f"Close {app} → [UI_ACTION:close_app:{app}]\n"
                ...
                if command.startswith("list_dir:"):
                    path = command.replace("list_dir:", "").replace(
                ...
                automation_context += (
                    f"Ask confirmation → [UI_ACTION:confirm_action:{command}]\n"
                )
    ...
    minimal_messages = (
        [Message(role="system", content=system_content, timestamp="")]
        + history
    ...
    if search_needed:
        ...
        try:
            search_results = await asyncio.wait_for(
                search_web(search_query_used, max_results=5), timeout=10.0
            )
            if search_results:
                search_context = (
                ...
                minimal_messages.insert(
                    -1, Message(role="system", content=search_context, timestamp="")
                )
    ...
    if cascade["status"] == "ok":
        ...
        if fallback_occurred and failed_provider:
            response_text = (
                build_fallback_note(failed_provider, provider_used) + response_text
            )
        print(
            f"[VOICE] Used provider: {provider_used}"
            + (f" (after {failed_provider} failed)" if fallback_occurred else "")
        )
    elif cascade["status"] == "asking":
        response_text = build_ask_message(
            cascade.get("failed_provider") or "unknown", cascade.get("remaining") or []
        )
        provider_used = "asking"
        model_used = "asking"
    elif cascade["status"] == "override_unavailable":
        response_text = build_override_unavailable_message(
            cascade.get("failed_provider") or "unknown"
        )
    ...
    await memory_manager.extract_and_save_memories(text, conversation_id)
...
def trim_messages_to_budget(
    messages: list[Message], max_tokens: int = 4000
) -> list[Message]:
    """
    ...
    while total_tokens > max_tokens:
        ...
        print(
            f"[WARNING] Universal payload token budget exceeded ({total_tokens} > {max_tokens}). Trimming oldest history message."
        )
        msgs.pop(target_idx)
...
def needs_foreground_search(message: str) -> bool:
...
def build_foreground_url(message: str) -> dict:
    msg_lower = message.lower().strip()
    ...
    for t in yt_triggers:
        if t in msg_lower:
            ...
            for phrase in phrases_to_remove:
                ...
                query = re.sub(
                    re.escape(phrase), "", query, flags=re.IGNORECASE
                ).strip()
...
def is_file_system_command(message: str) -> tuple[bool, dict]:
    """
    ...
    for trigger in list_triggers:
        if trigger in msg_lower:
            for folder_name, folder_path in list_patterns.items():
    ...
    for trigger in create_triggers:
        if trigger in msg_lower:
            ...
            if match:
                ...
                base_path = list_patterns.get(location, desktop)
...
def needs_automation(message: str) -> bool:
    ...
    for q in question_patterns:
        if msg_lower.startswith(q) and (
            "available" in msg_lower or "exist" in msg_lower or "support" in msg_lower
        ):
            return False
...
async def get_automation_context_str(automation_results: list[dict]) -> str:
    ...
    if len(automation_context) > 380:
        ...
        automation_context = f"[TASK] Include: {first_line} (+{len(lines) - 1} more)"

    return automation_context


def deduplicate_actions(results: list[dict]) -> list[dict]:
    if not results:
        return results
    ...
    for r in results:
        if r.get("action_type") == "OPEN_URL":
            browser = r.get("browser", "firefox").lower().strip()
            # Normalize browser names
            if any(x in browser for x in ["firefox", "mozilla"]):
                browsers_opening_url.add("firefox")
            elif any(x in browser for x in ["edge", "microsoft"]):
                browsers_opening_url.add("edge")
            elif any(x in browser for x in ["chrome", "google"]):
    ...
    for r in results:
        ...
        app_name = r.get("command", "").lower().strip()

        is_duplicate_browser = False

        if "firefox" in browsers_opening_url or "mozilla" in browsers_opening_url:
            if any(x in app_name for x in ["firefox", "mozilla"]):
                is_duplicate_browser = True

        if "edge" in browsers_opening_url:
            if any(x in app_name for x in ["edge", "msedge"]):
                is_duplicate_browser = True

        if "chrome" in browsers_opening_url:
            if any(x in app_name for x in ["chrome"]):
                is_duplicate_browser = True
...
def remove_duplicate_apps(results: list[dict]) -> list[dict]:
    seen_apps = set()
    seen_urls = set()
    filtered = []
...
async def generate_automation_command(message: str, groq_api_key: str) -> list[dict]:
    try:
        ...
        response = await asyncio.to_thread(
            ...
            messages=[
                {
                    "role": "user",
                    "content": COMMAND_GENERATION_PROMPT.replace("{request}", message),
                }
            ],
            max_tokens=500,
            temperature=0,
...
async def chat_endpoint(request: ChatRequest):
    ...
    if conversation_id_exists:
        ...
        if system_msgs:
            ...
            search_context_accumulated = "\n\n" + "\n".join(
                m.content for m in recent_system_msgs
            )
            search_context_accumulated += "\n" + SEARCH_STRICT_INSTRUCTION
    ...
    new_user_message = Message(role="user", content=request.message, timestamp="")
    ...
    if is_file_cmd:
        ...
        minimal_messages = [
            Message(
                ...
                content=get_system_prompt(
                    personality_mode, modifier, address_preference
                ),
                timestamp="",
            ),
            Message(role="system", content=fs_context, timestamp=""),
            Message(role="user", content=request.message, timestamp=""),
        ]
        ...
        if needs_automation(request.message) and settings.GROQ_API_KEY:
            automation_results = await generate_automation_command(
                request.message, settings.GROQ_API_KEY
            )

        if automation_results:
            automation_context = await get_automation_context_str(automation_results)
            full_messages.insert(
                -1, Message(role="system", content=automation_context, timestamp="")
            )

    # Check if this is a pure automation command that doesn't need AI (voice_input)
    is_pure_automation = len(automation_results) > 0 and all(
        r.get("action_type")
        in ["OPEN_APP", "OPEN_URL", "SYSTEM_CONTROL", "lock_screen"]
        for r in automation_results
    )

    if is_pure_automation:
        # Build response without AI
        descriptions = [r.get("description", "Done") for r in automation_results]
        ...
        if len(descriptions) == 1:
            response_text = (
                f"{descriptions[0]}{_address_suffix(address_preference)}. {action_tags}"
            )
        else:
            items = ", ".join(descriptions[:-1])
            items += f" and {descriptions[-1]}"
            response_text = (
                f"Opening {items}{_address_suffix(address_preference)}. {action_tags}"
            )

        response_text = briefing_prefix + response_text

        # Broadcast and return
        await broadcast_voice_event({"type": "voice_input", "text": request.message})
        await broadcast_voice_event({"type": "voice_response", "text": response_text})
        ...
        def speak_direct():
            clean = re.sub(r"\[UI_ACTION:[^\]]*\]", "", response_text).strip()
            if clean:
    ...
    if needs_foreground_search(request.message):
        foreground_result = build_foreground_url(request.message)
    ...
    if search_needed:
        ...
        try:
            ...
            if search_results:
                ...
                search_context_note = f"[Search results for: {search_query_used}]\n"
                for r in search_sources[:3]:
                    search_context_note += f"- {r['title']} ({r.get('source', '')})\n"
                await save_message(
                    conversation_id=conversation_id,
                    role="system",
    ...
    try:
        ...
        if cascade["status"] == "ok":
            ...
            failed_provider = cascade.get("failed_provider")
            print(
                f"[CHAT] Successfully got response from {provider_used}"
                + (f" (after {failed_provider} failed)" if fallback_occurred else "")
            )
        elif cascade["status"] == "asking":
            response_text = build_ask_message(
                cascade.get("failed_provider") or "unknown",
                cascade.get("remaining") or [],
            )
            provider_used = "asking"
            model_used = "asking"
        elif cascade["status"] == "override_unavailable":
    ...
    await memory_manager.extract_and_save_memories(request.message, conversation_id)
...
async def chat_stream_endpoint(request: ChatRequest):
    try:
        ...
        available_providers = ", ".join(
            p.name.title() for p in provider_manager.providers
        )
        system_state = f"\n<SYSTEM_STATE>\nActive AI Brain (Provider): {active_provider.name.title()}\nActive Model: {active_provider.model}\nAvailable Brains: {available_providers}\n</SYSTEM_STATE>\n"

        personality_mode = await get_setting(
        ...
        ui_reminder = Message(role="system", content=UI_ACTION_REMINDER, timestamp="")
        ...
        if is_file_cmd:
            ...
            if automation_results:
                automation_context = await get_automation_context_str(
                    automation_results
                )
                full_messages.insert(
                    -1, Message(role="system", content=automation_context, timestamp="")
        ...
        if is_pure_automation:
            ...
            async def generate_direct():
                ...
                for word in response_text.split():
                    yield (
                        json_module.dumps({"type": "token", "content": word + " "})
                        + "\n"
                    )
            ...
            return StreamingResponse(
                generate_direct(), media_type="application/x-ndjson"
            )
        ...
        async def generate():
            ...
            try:
                ...
                if search_needed:
                    yield (
                        json_module.dumps(
                            {"type": "search_started", "query": search_query_used}
                        )
                        + "\n"
                    )
                    ...
                    try:
                        ...
                        if search_results:
                            ...
                            full_messages.insert(
                                -1,
                                Message(
                                    role="system", content=search_context, timestamp=""
                                ),
                            )
                ...
                try:
                    ...
                    full_messages = trim_messages_to_budget(
                        full_messages, max_tokens=4000
                    )
                    ...
                    (
                        override,
                        fallback_mode,
                    ) = await fallback_module.get_provider_settings()
                    one_shot_provider = await fallback_module.consume_awaiting_choice(
                        request.message
                    )
                    ...
                    if override:
                        matched = next(
                            (p for p in providers_to_try if p.name == override), None
                        )
                    ...
                    print(
                        f"[STREAM] Attempting providers: {[p.name for p in providers_to_try]}"
                    )
                    ...
                    for provider in providers_to_try:
                        if not await provider.is_available():
                            print(
                                f"[STREAM] Provider {provider.name} not available, skipping"
                            )
                        ...
                        print(
                            f"[STREAM] Using provider: {provider.name}, model: {provider.model}"
                        )
                        ...
                        try:
                            ...
                            print(
                                f"[STREAM] Successfully completed with {provider.name}"
                            )
                            break
                        except asyncio.CancelledError:
                            # Client disconnected - stop generating tokens to save API credits
                            print(
                                f"[STREAM] Client disconnected, stopping generation (saved {len(full_response_parts)} tokens)"
                            )
                            break
                            ...
                            if stream_started:
                                ...
                                print(
                                    f"[STREAM] Completing with partial response from {provider.name}"
                                )
                                break
                            failed_providers.append(provider.name)
                            if fallback_mode == "ask" and not override:
                            ...
                            print("[STREAM] Falling back to next provider")
                            full_response_parts = []  # Reset
                            continue

                    if override_unavailable_provider:
                        error_msg = build_override_unavailable_message(
                            override_unavailable_provider
                        )
                        full_response_parts = [error_msg]
                        provider_used_name = "override_unavailable"
                        model_used_name = "unavailable"
                        yield (
                            json_module.dumps({"type": "token", "content": error_msg})
                            + "\n"
                        )
                    elif asking_message:
                        ...
                        yield (
                            json_module.dumps(
                                {"type": "token", "content": asking_message}
                            )
                            + "\n"
                        )
                    # If no provider succeeded at all
                    elif not stream_started and not full_response_parts:
                        print("[STREAM] All providers failed, sending error token")
                        if provider_manager.is_unconfigured():
                    ...
                    print("[STREAM] Request cancelled by client during streaming")
                ...
                new_response = await detect_and_log_gap(
                    complete_response, request.message
                )
                if len(new_response) > len(complete_response):
                    gap_note = new_response[len(complete_response) :]
                    yield (
                        json_module.dumps({"type": "token", "content": gap_note}) + "\n"
                    )
                    ...
                    try:
                        ...
                        async def speak_and_broadcast():
                            ...
                            if fallback_occurred and failed_provider_name:
                                ...
                                clean = (
                                    build_fallback_note(
                                        failed_provider_name, provider_used_name
                                    )
                                    + clean
                                )
                ...
                try:
                    yield (
                        json_module.dumps({"type": "error", "message": str(fatal_err)})
                        + "\n"
                    )
                except:
                    pass
...
async def google_auth_callback(code: str):
    ...
    return HTMLResponse(
        "<script>window.close();</script><h1>Authentication successful! You can close this tab.</h1>"
    )
...
def _check_plugin(plugin_id: str):
    ...
    if not registry.is_configured(plugin_id):
        raise HTTPException(
            status_code=503, detail=f"Plugin {plugin_id} is not configured."
        )
...
async def create_calendar_event(req: CalendarCreateRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=503, detail=f"Calendar API error: {e.response.text}"
        )
    except Exception as e:
...
async def resolve_gap(gap_id: str):
    ...
    try:
        async with aiosqlite.connect(settings.DB_PATH) as db:
            await db.execute(
                "UPDATE gap_log SET resolved = 1 WHERE gap_id = ?", (gap_id,)
            )
            await db.commit()
