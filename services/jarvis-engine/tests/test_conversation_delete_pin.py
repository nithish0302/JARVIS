...
from jarvis_engine.memory.conversation import get_conversation_messages, save_message
...
async def test_delete_conversation_rejected_without_valid_pin():
    ...
    try:
        with TestClient(app) as client:
            ...
            wrong_pin = client.delete(
                f"/conversation/{conversation_id}", params={"pin": "0000"}
            )
        ...
        with TestClient(app) as client:
            ...
            client.delete(
                f"/conversation/{conversation_id}", params={"pin": stored_pin}
            )
