...
async def test_edit_reembeds_on_content_change():
    memory_id = await memory_manager.save_memory(
        content=_unique_content("original unrelated sentence about weather"),
        importance=5,
    )
    try:
        new_content = _unique_content(
            "a totally different sentence about spacecraft propulsion"
        )
