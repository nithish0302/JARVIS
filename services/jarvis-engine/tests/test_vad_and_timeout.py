...
def test_speech_recorder_pause_before_speaking():
    ...
    assert recorded_duration > 4.0, (
        f"Expected recording > 4.0s including speech, got {recorded_duration:.2f}s"
    )
...
def test_speech_recorder_timeout_anchors_to_tts_finished_event():
    ...
    assert len(audio) == 0, (
        "Should still give up once TTS is done and 6s of silence passes"
    )
