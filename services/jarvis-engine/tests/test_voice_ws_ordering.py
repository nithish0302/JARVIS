    ...
    with TestClient(app) as client:
        with client.websocket_connect("/ws/voice") as ws:
            ...
            for idx, (text, direct_resp) in enumerate(commands, 1):
                ...
                res = client.post("/voice/input", json={"text": text, "direct_response": direct_resp})
                assert res.status_code == 200
                ...
                while len(events) < 6:
                    ...
                    assert seq > last_seq, f"Sequence not monotonically increasing: {seq} <= {last_seq}"
                    last_seq = seq
                ...
                expected_order = ["listening", "processing", "voice_input", "voice_response", "speaking", "idle"]
                assert seq_summary == expected_order, f"Order mismatch in command {idx}: {seq_summary} != {expected_order}"
                assert seq_summary[-1] == "idle", f"'idle' must be last in command {idx}"
                assert "idle" not in seq_summary[:-1], f"'idle' appeared prematurely in command {idx}: {seq_summary}"
